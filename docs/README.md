...
- Guide Database - Browse native base/mechanism/material pages that may use DPDNS as the more complete canonical source, plus supplemental class guides
...
The app uses a single HTM-based data system. Core item data is fetched from the ESR documentation site (`easternsunresurrected.com`), selected base/mechanism/material guide pages use DPDNS as the more complete canonical source, and everything is parsed with the native `DOMParser` API into one IndexedDB database (`d2r-esr-runeword-browser`).
...
| Guide Database | Native base information, material, and mechanism pages, with DPDNS used as canonical source where it is more complete, plus supplemental class guides |
...
| Guide Database | `/database` | Base information, DPDNS-completed native pages, materials, mechanisms, and class guides rendered in a searchable Chinese app UI with compact notes, counted row marker filters, and categorized material/affix color cues |
