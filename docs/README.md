...
- Guide Database - Browse native base/mechanism pages that may use DPDNS as the more complete canonical source, plus supplemental guides
...
The app uses a single HTM-based data system. Core item data is fetched from the ESR documentation site (`easternsunresurrected.com`), selected base/mechanism guide pages use DPDNS as the more complete canonical source, and everything is parsed with the native `DOMParser` API into one IndexedDB database (`d2r-esr-runeword-browser`).
...
| Guide Database | Native base information and mechanism pages, with DPDNS used as canonical source where it is more complete, plus supplemental materials, endgame pages, and class guides |
...
| Guide Database | `/database` | Base information, DPDNS-completed native pages, and supplemental guide data rendered in a searchable Chinese app UI with compact notes plus categorized material and affix color cues |
