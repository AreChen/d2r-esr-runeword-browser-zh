...
Guide page table rendering keeps the parsed text intact for search/filtering, then applies display-only color cues to recognizable material requirements and affix/stat lines in table cells. The same semantic markers power guide row filters, so users can quickly narrow formulas to rows containing runes, gems, corruption materials, organs, D-Stoning/Dragon Stone materials, forging materials, aura stones, socket materials, map/key materials, generic cube materials, consumables, coupon/decipherer-style currency, or affix/stat outcomes. Affix markers keep the broad “affix/stat outcome” filter while also exposing common families such as skill bonuses, resistances, damage/piercing outcomes, speed breakpoints, and chance-to-cast triggers. Marker options are counted from the current guide page and hidden when no rows match that marker; when search, level, section, row favorite, or marker filters are active, marker counts are recalculated against those conditions while ignoring the marker filter itself. Section filter badges show current matching rows against total rows when search, level, favorites, or marker filters are active. Formula rows can also be favorited directly, so “favorites only” can show concrete saved recipes instead of only whole sections. This keeps large formula pages focused on useful filters.
